...
function getNameUsingDestructuring(person) {
  ...
  const fullNames = getNamesUsingDestructuring(firstName, lastName) 
  return fullNames
}
...
function translateToPirateTalk(phrase) {
  const wordsOnPhrase = phrase.split('')

  const translateWords = words.map(word => {
    if (wordsOnPhrase === EN_PIRATE_LOOKUP){
      return EN_PIRATE_LOOKUP[wordsOnPhrase]
